...
try {
    ...
    if ($ok !== false) {
        // Log activity: BHW/Midwife rejected and removed Baby Card request
        try {
            supabaseLogActivity(
                $approver_id,
                $approver_type,
                'BABYCARD_DOC_REQUEST_REJECTED',
                $approver_name . ' rejected and removed Baby Card request for ' . $child_name . ', child of ' . $parent_name . ' (Request ID: ' . $request_id . ')',
                $_SERVER['REMOTE_ADDR'] ?? null
            );
        } catch (Exception $e) {
            // Log error but don't fail the rejection
            error_log('Failed to log Baby Card request rejection activity: ' . $e->getMessage());
        }
        
        echo json_encode([
            'status' => 'success',
            'message' => 'Baby Card request rejected and removed successfully'
        ]);
    } else {

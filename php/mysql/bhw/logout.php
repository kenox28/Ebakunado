...
try {
    ...
    if ($connect && !$connect->connect_error) {
        ...
        if ($log_stmt) {
            ...
            $log_stmt->bind_param("ssss", $bhw_id, 'bhw', $description, $ip);
            $log_stmt->execute();
            $log_stmt->close();
            error_log("BHW logout logged successfully for ID: " . $bhw_id);
        } else {
...
echo json_encode([
    "status" => "success",
    "message" => "BHW logged out successfully",
    "debug" => [
        "db_connected" => $db_connected,
        "had_session" => ($bhw_id !== null)
    ]
]);

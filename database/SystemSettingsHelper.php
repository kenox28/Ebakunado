...
function getOTPCredentials() {
    try {
        // Get SuperAdmin's settings - get all to see what's available
        $settings = supabaseSelect('system_settings', 'sms_api_key,sms_device_id,user_id,updated_at', [
            'user_type' => 'super_admin'
        ], 'updated_at DESC', 1);
        
        if ($settings && count($settings) > 0) {
            $apiKey = trim($settings[0]['sms_api_key'] ?? '');
            $deviceId = trim($settings[0]['sms_device_id'] ?? '');
            
            return [
                'api_key' => $apiKey ?: '859e05f9-b29e-4071-b29f-0bd14a273bc2',
                'device_id' => $deviceId ?: '687e5760c87689a0c22492b3'
            ];
        }
        return [
...
/**
 * Get TextBee credentials for Daily Notifications (SuperAdmin's credentials)
 * 
 * @return array Returns ['api_key' => '', 'device_id' => ''] or default values
 * 
 * Note: Daily notifications now use the same SuperAdmin credentials as OTP
 * for centralized management and consistency.
 */
function getNotificationCredentials() {
    try {
        // Get SuperAdmin's settings (same as OTP credentials)
